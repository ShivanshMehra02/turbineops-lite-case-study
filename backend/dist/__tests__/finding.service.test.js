...
function mockResolved(fn, value) {
    fn.mockResolvedValue(value);
}
function mockRejected(fn, err) {
...
describe('finding.service', () => {
    ...
    it('lists with pagination + filters', async () => {
        ...
        expect(findingMocks.findMany).toHaveBeenCalledWith(expect.objectContaining({
            where: {
                inspectionId: 'i1',
                category: FindingCategory.BLADE_DAMAGE,
                severity: 3,
    ...
    it('validates merged state on update', async () => {
        ...
        await expect(updateFinding(db, 'f1', {
            category: FindingCategory.BLADE_DAMAGE,
    ...
    it('maps P2025 on delete', async () => {
        mockRejected(findingMocks.delete, new Prisma.PrismaClientKnownRequestError('x', { code: 'P2025', clientVersion: 't' }));
